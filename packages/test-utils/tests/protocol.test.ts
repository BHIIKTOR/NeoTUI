...
test("protocol writer rejects unsafe hyperlink and image payload input", () => {
  ...
  expect(() => writer.openHyperlink("javascript:alert(1)")).toThrow(
    "Unsupported hyperlink protocol: javascript:",
  );
  expect(() => writer.openHyperlink("https://opentui.com/\u001b[31m")).toThrow(
    "Invalid hyperlink: control characters are not allowed",
  );
  expect(() =>
    writer.writeKittyImage({
      imageId: -1,
